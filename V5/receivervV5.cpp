...
std::string hexToText(const std::string& hexString) {
    ...
    for (size_t i = 0; i < hexString.length(); i += 2) {
        // Extract each pair of hexadecimal characters
        ...
        ss << std::hex << hexPair;
        ss >> asciiValue;
        // Append the ASCII character to the text stream
        textStream << static_cast<char>(asciiValue);
    }
...
int main() {
    ...
    while (true) {
        ...
        std::vector<unsigned char> bytes = hexToText(hexData);
